...
const options = {
	...
	string: ['image', 'filter'],
	default: { preserve: true, marquee: true, video: true },
...
const api = async function (
	...
	if (destination && fs.statSync(dir).isDirectory()) {
		...
		const targets = fs
			.readdirSync(dir)
			.filter(
				e => e !== media && e !== xml && path.resolve(destination) !== path.resolve(dir, e)
			);
